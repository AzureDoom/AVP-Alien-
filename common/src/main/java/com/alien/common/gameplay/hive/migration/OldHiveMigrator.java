...
public final class OldHiveMigrator {
    ...
    public static int run(MinecraftServer server) {
        ...
        if (legacyHiveIds.isEmpty()) {
            Alien.LOGGER.info("Hive migrator: no legacy hive factions found — skipping");
            return 0;
        }

        Alien.LOGGER.info("Hive migrator: found {} legacy hive faction(s) — beginning migration", legacyHiveIds.size());
        ...
        Alien.LOGGER.info(
            "Hive migrator: migrated {}/{} legacy hives; mapping written to migration log",
            migratedCount,
            legacyHiveIds.size()
        );
    ...
    private static MigrationLog.MigrationEntry migrateOne(MinecraftServer server, ResourceLocation legacyId) {
        ...
        if (legacyFaction == null) {
            Alien.LOGGER.warn("Hive migrator: legacy hive {} resolves to null faction — skipping", legacyId);
            return null;
        }
        if (!(legacyFaction.data() instanceof HiveFactionData legacyData)) {
            Alien.LOGGER.warn(
                "Hive migrator: legacy hive {} has data type {}, expected HiveFactionData — skipping",
                legacyId,
        ...
        if (serverLevel == null) {
            Alien.LOGGER.warn(
                "Hive migrator: legacy hive {} dimension {} not loaded — skipping",
                legacyId,
                dimension.location()
            );
            return null;
        ...
        if (lineageData == null) {
            Alien.LOGGER.warn("Hive migrator: lineage {} data null after creation — aborting hive {}", lineageId, legacyId);
            return null;
        }

        com.alien.common.gameplay.hive.faction.FactionAesthetics.applyDefaults(
            lineageFaction,
            variant,
            com.alien.common.gameplay.hive.faction.FactionAesthetics.Tier.LINEAGE
        );
        ...
        lineageFaction.setName(
            com.alien.common.gameplay.hive.faction.FactionNaming.forLineage(variant, lineageNumber)
        );
        ...
        Alien.LOGGER.info(
            "Hive migrator: migrated legacy hive {} → lineage {} + location {} ({} chunks claimed, {} members)",
            legacyId,
            lineageId,
            locationId,
    ...
    private static void claimChunksWithinRadius(
        ...
        for (var dx = -LEGACY_HIVE_RADIUS_CHUNKS; dx <= LEGACY_HIVE_RADIUS_CHUNKS; dx++) {
            for (var dz = -LEGACY_HIVE_RADIUS_CHUNKS; dz <= LEGACY_HIVE_RADIUS_CHUNKS; dz++) {
                ...
                // Skip if some other location already owns this chunk (shouldn't happen on a fresh hive system,
                // but defensive).
                if (HiveLocationRegistry.INSTANCE.getByChunk(level.dimension(), chunk) != null) {
                    continue;
                }
                com.alien.common.gameplay.hive.growth.HiveLocationClaims.claim(level, location, chunk, currentTick);
            }
        }
    }
