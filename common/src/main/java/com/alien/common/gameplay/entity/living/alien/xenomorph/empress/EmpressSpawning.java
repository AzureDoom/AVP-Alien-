...
public class EmpressSpawning {

    private static final int MAX_OVERWORLD_Y_LEVEL = -24;

    public static final SpawnPlacements.SpawnPredicate<Empress> PREDICATE = EmpressSpawning::canSpawn;

    public static <T extends Empress> SpawnPlacements.SpawnPredicate<T> predicate() {
        return EmpressSpawning::canSpawn;
    }

    private static <T extends Empress> boolean canSpawn(
        ...
        if (
            EmpressSpawnChunkData.getOrCreate(serverLevel).isSomeAnd(empressSpawnChunkData -> empressSpawnChunkData.getSpawnCooldown().isActive())
        ) {
            return false;
        }

        var empressSpawnChunkDataOption = EmpressSpawnChunkData.getOrCreate(serverLevel);
        var isChunkSpawnAvailable = empressSpawnChunkDataOption
            .isSomeAnd(empressSpawnChunkData -> !empressSpawnChunkData.isChunkBlacklisted(blockPos));
    ...
    private static boolean canStrainSpawnInLevel(ServerLevel serverLevel, EntityType<?> entityType) {
