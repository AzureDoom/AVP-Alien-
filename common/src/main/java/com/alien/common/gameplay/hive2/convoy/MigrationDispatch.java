...
public final class MigrationDispatch {
    ...
    private static boolean tryDispatch(
        ...
        if (destination == null) {
            Alien.LOGGER.info(
                "Hive2: migration declined for {} — no sister destination in lineage {}",
                source.id(),
                lineageFactionId
            );
            return false;
        ...
        if (serverLevel == null) {
            Alien.LOGGER.warn(
                "Hive2: migration declined for {} — dimension {} not loaded",
                source.id(),
        ...
        Alien.LOGGER.info(
            "Hive2: migration dispatched: {} → {} carrying {} members + {} biomass{}",
            source.id(),
            destination.id(),
