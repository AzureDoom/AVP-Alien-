 ...
 * {@link net.minecraft.client.renderer.entity.ItemRenderer#renderStatic} with
 * {@link ItemDisplayContext#FIXED}, which is the closest semantic for "stationary item display in the
 * world." The user can tune {@code FIXED} via the regular transform-tune command to fit it to a block.
 * <p>
 * Floor blocks rotate around Y based on the {@code ROTATION_16} blockstate (16-step yaw); wall blocks
 * pick a 90° increment from {@code FACING}, plus a translate so the head visually sticks out of the
 * wall instead of sitting at the block's volumetric center.
 */
