...
/**
 * Writes the legacy-hive → new-(lineage, location) mapping to {@code data/avp_alien/hive_migration.log} in the world
...
public final class MigrationLog {
    ...
    private static final String LOG_FILE_NAME = "hive_migration.log";
    ...
    public static void write(MinecraftServer server, Map<ResourceLocation, MigrationEntry> entries) {
        ...
        try {
            ...
            Alien.LOGGER.warn("Hive migrator: failed to create migration log directory {}: {}", dataPath, e.getMessage());
            return;
        }
        ...
        try (var writer = Files.newBufferedWriter(logPath)) {
            ...
            Alien.LOGGER.info("Hive migrator: wrote {} migration entries to {}", entries.size(), logPath);
        } catch (IOException e) {
            Alien.LOGGER.warn("Hive migrator: failed to write migration log {}: {}", logPath, e.getMessage());
        }
    }

    private static void writeHeader(BufferedWriter writer, int entryCount) throws IOException {
        writer.write("# AVP-Alien hive migration log");
        writer.newLine();
